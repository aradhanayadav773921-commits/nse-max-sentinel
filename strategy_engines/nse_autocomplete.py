...
"""Helpers for NSE stock autocomplete inputs across the Streamlit UI."""
...
_NSE_COMPANY_NAMES: dict[str, str] = {
    ...
    "AVAILFC": "Available Finance Limited",
    ...
    "IGIL": "International Gemmological Institute (India) Limited",
...
_MANUAL_COMPANY_NAMES: dict[str, str] = {
    "ADANITRANS": "Adani Energy Solutions Limited",
    "AARTI": "Aarti Industries Limited",
    "APAR": "Apar Industries Limited",
    "ASTER": "Aster DM Healthcare Limited",
    "BALKRISHIND": "Balkrishna Industries Limited",
    "BIRLASOFT": "Birlasoft Limited",
    "BSEINDIA": "BSE Limited",
    "BURGERKING": "Restaurant Brands Asia Limited",
    "CAPLIN": "Caplin Point Laboratories Limited",
    "EIL": "Engineers India Limited",
    "FINOLEX": "Finolex Industries Limited",
    "GATI": "Allcargo Gati Limited",
    ...
    "KALPATPOWR": "Kalpataru Projects International Limited",
    ...
    "MAHINDLOG": "Mahindra Logistics Limited",
    "MAXFINSERV": "Max Financial Services Limited",
    "MCDOWELL-N": "United Spirits Limited",
    "MINDSPACE": "Mindspace Business Parks REIT",
    "MOFSL": "Motilal Oswal Financial Services Limited",
    "MOLDTEK": "Mold-Tek Packaging Limited",
    "MTAR": "MTAR Technologies Limited",
    "NARAYANAH": "Narayana Hrudayalaya Limited",
    "NATCOPHARMA": "Natco Pharma Limited",
    "NSDL": "National Securities Depository Limited",
    "RELINFRA": "Reliance Infrastructure Limited",
    "REPCO": "Repco Home Finance Limited",
    "SHILPA": "Shilpa Medicare Limited",
    "SIYARAM": "Siyaram Silk Mills Limited",
    "SKF": "SKF India Limited",
    "SRTRANSFIN": "Shriram Finance Limited",
    "STRIDES": "Strides Pharma Science Limited",
    "TATAMOTORS": "Tata Motors Limited",
    "TIPS": "Tips Industries Limited",
    "VINATI": "Vinati Organics Limited",
...
def _prettify_symbol(symbol: str) -> str:
    text = symbol.replace("-", " ").replace("&", " & ")
    return " ".join(part for part in text.split()) or symbol


def _company_name_for_symbol(symbol: str) -> str:
    return (
        _NSE_COMPANY_NAMES.get(symbol)
        or _MANUAL_COMPANY_NAMES.get(symbol)
        or _prettify_symbol(symbol)
    )
...
def configure_nse_stock_search(tickers: Sequence[str] | None) -> None:
    ...
    if not symbols:
        for symbol in _NSE_COMPANY_NAMES:
            if symbol not in seen:
                seen.add(symbol)
                symbols.append(symbol)

    universe_key = tuple(symbols)
    if universe_key == _SEARCH_UNIVERSE_KEY:
        return
...
def _match_rows(query: str) -> list[tuple[int, str, str]]:
    needle = query.strip().lower()
    if not needle:
        return []

    starts_with: list[tuple[int, str, str]] = []
    contains: list[tuple[int, str, str]] = []
    for symbol, company, symbol_lc, company_lc in _SEARCH_ROWS:
        if needle not in symbol_lc and needle not in company_lc:
            continue
        formatted = f"{symbol}{_DISPLAY_SEPARATOR}{company}"
        bucket = starts_with if symbol_lc.startswith(needle) or company_lc.startswith(needle) else contains
        bucket.append((0, symbol, formatted))

    starts_with.sort(key=lambda item: item[1])
    contains.sort(key=lambda item: item[1])
    return starts_with + contains


def search_nse_stocks(query: str) -> list[str]:
    """Return up to 10 formatted suggestions for the current NSE universe."""
    return [formatted for _, _, formatted in _match_rows(query)[:10]]


def extract_selected_symbol(value: str | None) -> str:
    raw_value = str(value or "").strip()
    if not raw_value:
        return ""

    for separator in (_DISPLAY_SEPARATOR, " ? ", " - "):
        if separator in raw_value:
            raw_value = raw_value.split(separator, 1)[0]
            break

    normalized = _normalize_symbol(raw_value)
    if normalized:
        active_symbols = {symbol for symbol, _, _, _ in _SEARCH_ROWS}
        if normalized in active_symbols:
            return normalized

    lowered = raw_value.lower()
    for symbol, company, _symbol_lc, company_lc in _SEARCH_ROWS:
        if lowered == company_lc:
            return symbol

    matches = _match_rows(raw_value)
    if len(matches) == 1:
        return matches[0][1]
